...
test("Ocean-v1.3.3.2 前后端正式发布元数据使用同一基线", () => {
  assert.equal(pkg.version, "1.3.3-2");
  assert.equal(RELEASE_VERSION, "1.3.3-2");
  assert.equal(RELEASE_STAGE, "Ocean-v1.3.3.2");
  assert.equal(RULE_VERSION, "1.8");
  assert.equal(SOCKET_PROTOCOL_VERSION, "2.1");
  assert.deepEqual(Data.RELEASE, {
    version: "1.3.3-2",
    stage: "Ocean-v1.3.3.2",
    ruleVersion: "1.8",
...
test("当前文档冻结 v1.3 教程、人机边界与 v1.3.3 真实素材音效范围", () => {
  const currentDocs = [
    ...
    read("docs/release-manifest-Ocean-v1.3.3.2.md"),
  ].join("\n");
