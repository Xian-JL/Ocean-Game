...
test("v0.7.6 前后端发布元数据使用同一基线", () => {
  assert.equal(pkg.version, "0.7.6");
  assert.equal(RELEASE_VERSION, "0.7.6");
  assert.equal(RELEASE_STAGE, "postlaunch-v0.7.6");
...
test("当前规则与页面流程文档不再保留已废弃的核心数值", () => {
  const currentDocs = [
    ...
    read("docs/release-manifest-postlaunch-v0.7.6.md"),
  ].join("\n");
