# 《海战 OCEAN》Socket.IO 协议 v0.7

> 实现版本：postlaunch-v0.6  
> 协议版本：1.4  
> 依据：rule-v1.3.md、page-flow-v1.3.md  
...
| `final-salvo:submit` | `{ expectedVersion, decoyId }` | `FINAL_SALVO` 选择一枚尚未触发的己方诱饵；选择对对手保密，双方提交后同时结算 |
...
| `FINAL_SALVO` | `room:leave` 拒绝；玩家通过 `final-salvo:submit` 逐轮完成终局鱼雷选择 |
...
协议 1.4 新增 `room:create.maxPlayers`（2 或 3）、三人行动的 `intent.targetPlayerId`、视图字段 `opponentIds`/`enemyMapsByPlayer`，以及行动方、防守方和第三方三类反馈。旧版接口记录继续保留；现行实现以本文件的协议 1.4 为准。
